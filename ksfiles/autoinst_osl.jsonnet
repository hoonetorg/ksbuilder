{
  ...
  software: {
    patterns: ["kvm_tools", "gnome", "selinux",],
    packages: [],
  },
  ...
  // Pre-script runs BEFORE the storage proposal.
  // It guarantees partitions 1/2/3 exist on the disk so the static
  // storage section below can safely search by number with ifNotFound: "error".
  // No need to call 'agama config load' here — the static config is sufficient.
  scripts: {
    pre: [
      {
  ...
  // proposal runs after the pre-script.
  // All three partitions are guaranteed to exist at this point.
  storage: {
    boot: { configure: false, },
    drives: [{
      search: {
        condition: { name: "{{diskpath}}", },
        ifNotFound: "error",
      },
      ptableType: "gpt",
      partitions: [
        {
          // Part 1: EFI — reuse fs if it was pre-existing, format if freshly created
          search: {
            condition: { number: 1, },
            ifNotFound: "error",
          },
          ...
          filesystem: {
            path: "/boot/efi",
            type: "vfat",
            reuseIfPossible: true,   // pre-script formats it only when creating fresh
          },
        },
        {
